...
from architectures.BPR.bpr import BPR
from loaders.create_dataloader_bpr import CreateDataloader
...
def _reindex(ratings, offset=6863):
    ...
    #print('Type of map is', ratings.head(), ratings.dtypes)
...
if __name__ == '__main__':
    ...
    print(args.epochs, args.lr, args.dropout, args.batch_size, args.factor_num, args.layers, args.num_ng, args.num_ng_test, args.top_k)
    ...
    print('max_item_index', train_rating_data['item_id'].min(), train_rating_data['item_id'].max(), test_rating_data['item_id'].min(), test_rating_data['item_id'].max())

    graph_embeddings = np.load(MAIN_PATH + 'node_embeddings_gcf_128_rotate.npy')
    ...
    model = BPR(num_users, num_items, 128, 0.1)
    ...
    for epoch in range(1, args.epochs+1):
        ...
        #for user, item, label, graph_embeddings in tqdm(train_loader):
        for user, item_preferred, item_not_preferred in tqdm(train_loader):
            # print(user.size(), item.size(), label.size())
            # print(user, item, label)
            user = user.to(device)
            item_preferred = item_preferred.to(device)
            item_not_preferred = item_not_preferred.to(device)
            #graph_embeddings = graph_embeddings.to(device)


            optimizer.zero_grad()
            # print('Zero Grad')
            #print('Indexes are', np.max(user.numpy()), np.max(item.numpy()), num_users, num_items)
            #prediction = model(user, item, graph_embeddings)
            #prediction = model(user, item)
            # print('Prediction')
            loss = model(user, item_preferred, item_not_preferred)
            # print('Loss')
            loss.backward()
            # print('Backward')
            optimizer.step()
