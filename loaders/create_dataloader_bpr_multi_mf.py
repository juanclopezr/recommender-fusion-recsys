...
class CreateDataloader(object):
    ...
    """
    def __init__(self, args, train_ratings, test_ratings, dataset_path, with_text=True, tokenizations=None, graph_embeddings=None, user_bpr_embeddings=None, item_bpr_embeddings=None, num_users=6863):
        ...
        self.user_bpr_embeddings = user_bpr_embeddings
        self.item_bpr_embeddings = item_bpr_embeddings
    ...
    def collate_fn(self, batch):
        ...
        bpr_user_embeddings = torch.zeros((len(batch), self.bpr_user_embeddings.shape[1]))
        for i, x in enumerate(batch):
            bpr_user_embeddings = self.bpr_user_embeddings[x[0].item()]

        bpr_item_embeddings = torch.zeros((len(batch), self.bpr_item_embeddings.shape[1]))
        for i, x in enumerate(batch):
            bpr_item_embeddings = self.bpr_item_embeddings[x[1].item()]
