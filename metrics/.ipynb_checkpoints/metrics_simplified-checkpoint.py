...
def metrics(model, test_loader, top_ks, device, ng_num):
    ...
    with torch.no_grad():
        for user, item, label, graph_embeddings in tqdm(test_loader, total=len(test_loader)):
            user = user.to(device)
            item = item.to(device)
            ...
            for top_k in top_ks:
                ng_items, recommends, mrr_recommends = calculate_metrics_user(predictions, device, user, item, label, top_k, ng_num)
...
def calculate_metrics_user(predictions, device, user, item, label, top_k, ng_num=100):
    user = user.to(device)
    item = item.to(device)
    label = label.to(device)
